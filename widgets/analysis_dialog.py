...
class AnalysisDialog(BaseDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Interactive Endpoints Analysis")
        self.setMinimumSize(1200, 800)
        
        self.csv_files, self.analysis_thread, self.analysis_worker, self.results_data = [], None, None, []
        ...
        self.left_pane.addWidget(file_group)
        ...
        self.left_pane.addWidget(mode_group)
        ...
        self.left_pane.addWidget(self.side_view_group)
        
        self.top_view_group = QtWidgets.QGroupBox("Top View Parameters");
        self.left_pane.addWidget(self.top_view_group)

        self.side_view_endpoints_group = QtWidgets.QGroupBox("Side View Endpoints")
        side_endpoints_main_layout = QtWidgets.QVBoxLayout(self.side_view_endpoints_group)
        self.side_view_endpoints_scroll = QtWidgets.QScrollArea(); self.side_view_endpoints_scroll.setWidgetResizable(True)
        self.side_view_endpoints_widget = QtWidgets.QWidget(); self.side_view_endpoints_layout = QtWidgets.QFormLayout(self.side_view_endpoints_widget)
        self.side_view_endpoints_scroll.setWidget(self.side_view_endpoints_widget)
        side_endpoints_main_layout.addWidget(self.side_view_endpoints_scroll)
        ...
        threshold_layout.addRow("Rapid Threshold:", self.rapid_thresh_spinbox)
        threshold_layout.addRow("Freezing Threshold:", self.freezing_thresh_spinbox)
        side_endpoints_main_layout.addLayout(threshold_layout)
        self.left_pane.addWidget(self.side_view_endpoints_group)

        self.top_view_endpoints_group = QtWidgets.QGroupBox("Top View Endpoints")
        top_endpoints_main_layout = QtWidgets.QVBoxLayout(self.top_view_endpoints_group)
        self.top_view_endpoints_scroll = QtWidgets.QScrollArea(); self.top_view_endpoints_scroll.setWidgetResizable(True)
        self.top_view_endpoints_widget = QtWidgets.QWidget(); self.top_view_endpoints_layout = QtWidgets.QFormLayout(self.top_view_endpoints_widget)
        self.top_view_endpoints_scroll.setWidget(self.top_view_endpoints_widget)
        top_endpoints_main_layout.addWidget(self.top_view_endpoints_scroll)
        self.angular_thresh_spinbox = QtWidgets.QDoubleSpinBox(value=90, minimum=0, maximum=180, suffix=" deg/s")
        top_threshold_layout = QtWidgets.QFormLayout()
        top_threshold_layout.addRow("Angular Vel. Threshold:", self.angular_thresh_spinbox)
        top_endpoints_main_layout.addLayout(top_threshold_layout)
        self.left_pane.addWidget(self.top_view_endpoints_group)
        ...
        universal_layout.addRow("Frame Rate (FPS):", self.frame_rate_spinbox); universal_layout.addRow("Conversion Rate (px/cm):", self.conversion_rate_spinbox); self.left_pane.addWidget(universal_group)
        ...
        self.left_pane.addWidget(self.centroid_sliders_group)
    ...
    def setup_side_view_tank_widgets(self):
        ...
        for tank_num in sorted(self.geometric_centers.keys()):
            ...
            range_slider = RangeSlider(); range_slider.setValues(50, 50) 
            ...
            layout.addWidget(range_slider); self.side_view_params_layout.addWidget(group)
    ...
    def update_side_view_config(self, tank_num, key, value, slider_instance):
        if key == 'zones':
            ...
            if low + high > 100:
                ...
                slider_instance.setValues(last_config['zone1'], last_config['zone2']); return
    ...
    def add_csv_directory(self):
        ...
        if directory:
            ...
            if newly_found: self.csv_list_widget.addItems(newly_found); self.update_dynamic_endpoints()
    ...
    def clear_all_csvs(self):
        self.csv_list_widget.clear(); self.csv_files.clear(); self.update_dynamic_endpoints()
    def update_dynamic_endpoints(self):
        for layout in [self.side_view_endpoints_layout, self.top_view_endpoints_layout]:
            while layout.count():
                item = layout.takeAt(0)
                if item.widget(): item.widget().deleteLater()
        
        side_static = ["Average Speed (cm/s)", "Freezing Time (%)", "Swimming Time (%)", "Rapid Time (%)", "Time in Top (%)", "Time in Middle (%)", "Time in Bottom (%)", "Entries to Top", "Fractal Dimension", "Entropy"]
        top_static = ["Average Speed (cm/s)", "Average Distance from Center (cm)", "Average Angular Velocity (degree/s)", "Meandering (degree/m)", "Fractal Dimension", "Entropy"]
        unique_behaviors = set()
        if self.csv_files:
            QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
            for file_path in self.csv_files:
                try:
                    df = pd.read_csv(file_path, usecols=['class_name'])
                    unique_behaviors.update(df['class_name'].dropna().unique())
                except Exception as e: print(f"Could not read class names from {file_path}: {e}")
            QtWidgets.QApplication.restoreOverrideCursor()
        
        behavior_endpoints = [f"Time_{str(b).replace(' ', '_').capitalize()}_(%)" for b in sorted(list(unique_behaviors))]
        
        self.side_view_endpoints_checkboxes = {}
        for name in side_static + behavior_endpoints:
            cb = QtWidgets.QCheckBox(name, checked=True); self.side_view_endpoints_checkboxes[name] = cb; self.side_view_endpoints_layout.addRow(cb)
        
        self.top_view_endpoints_checkboxes = {}
        for name in top_static + behavior_endpoints:
            cb = QtWidgets.QCheckBox(name, checked=True); self.top_view_endpoints_checkboxes[name] = cb; self.top_view_endpoints_layout.addRow(cb)
    ...
    def load_analysis_settings(self):
        ...
        try:
            ...
            if self.geometric_centers: self.setup_centroid_sliders()
            self.update_visualization()
            if 'side_view_configs' in settings: self.side_view_tank_configs = {int(k): v for k,v in settings['side_view_configs'].items()}
            if self.geometric_centers: self.setup_side_view_tank_widgets()
