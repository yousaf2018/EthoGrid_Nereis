...
class EndpointsAnalyzer:
    ...
    def analyze(self):
        ...
        total_tracked_frames = len(self.df)
        ...
        # ### NEW LOGIC: Calculate Behavior Time Budgets ###
        if total_tracked_frames > 0 and 'class_name' in self.df.columns:
            behavior_counts = self.df['class_name'].value_counts()
            for behavior, count in behavior_counts.items():
                if pd.isna(behavior): continue
                # Sanitize the name for the column header
                clean_behavior_name = str(behavior).replace(' ', '_').capitalize()
                endpoint_name = f"Time_{clean_behavior_name}_(%)"
                self.results[endpoint_name] = (count / total_tracked_frames) * 100

        if analysis_mode == 'Side View':
            self._analyze_side_view(speeds, time_intervals, total_duration_of_tracking)
        ...
        final_results = {}
        # Filter results based on what the user selected in the UI
    ...
    def _analyze_side_view(self, speeds, time_intervals, total_duration):
        ...
        if axis == 'Top-Bottom':
            start_point, end_point = (p1 + p2) / 2, (p4 + p3) / 2
        elif axis == 'Left-Top to Right-Bottom':
            start_point, end_point = p1, p3
        else: # Left-Bottom to Right-Top
            start_point, end_point = p4, p2
        ...
        zone1_percent = self.params['zone1_percent'] / 100.0
        zone2_percent = self.params['zone2_percent'] / 100.0
        
        top_mask = (self.df['progress'] <= zone1_percent).to_numpy()
        bottom_mask = (self.df['progress'] >= (1.0 - zone2_percent)).to_numpy()
        middle_mask = (~top_mask) & (~bottom_mask)

        time_in_top = np.sum(top_mask) / self.params['frame_rate']
        time_in_middle = np.sum(middle_mask) / self.params['frame_rate']
        time_in_bottom = np.sum(bottom_mask) / self.params['frame_rate']
